    ...
    public class ModifyProject : BaseTask
    {
        ...
        public override bool Execute()
        {
                ...
                || (FilesToDeleteFromProject != null && FilesToDeleteFromProject.Length != 0))
            {
                ...
                LogVerbose($"IntermediateOutputPath : {IntermediateOutputPath}");
                //do not always save - it shouldn't end up in project if not necesary - messes up azure pipelines default build
                var withoutExt = Path.Combine(IntermediateOutputPath, Path.GetFileNameWithoutExtension(ProjectFileLoad));
